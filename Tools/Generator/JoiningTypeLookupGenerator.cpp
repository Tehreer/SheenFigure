...
void JoiningTypeLookupGenerator::generateFile(const std::string &directory) {
    ...
    element.fill();
    arrJoiningTypes.setSizeDescriptor(Converter::toString((int)element.index()));
    ranges.push_back({ start, previous, block });
    ...
    source.append(" *").newLine();
    source.append(" * REQUIRED MEMORY: " + Converter::toString((int)element.index()) + " Bytes").newLine();
    source.append(" */").newLine();
    source.newLine();
